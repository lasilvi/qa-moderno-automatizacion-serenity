...
**Objetivos específicos identificados en el código:**

1. **Digitalización del proceso de pedidos**: Eliminar la toma manual de pedidos mediante una interfaz web responsiva donde los clientes seleccionan mesa, productos del menú y confirman su orden.

2. **Comunicación asíncrona cocina-cliente**: Implementar un sistema de mensajería basado en eventos (RabbitMQ) que permita la actualización en tiempo real del estado de los pedidos sin acoplamiento directo entre servicios.

3. **Trazabilidad completa**: Mantener registro histórico de todos los pedidos con soft delete (nunca se eliminan físicamente), timestamps automáticos de creación/actualización, y proyecciones separadas para cocina y reportería.

4. **Reportería analítica**: Proporcionar métricas de negocio agregadas (ingresos totales, órdenes completadas, productos más vendidos) mediante arquitectura CQRS con base de datos desnormalizada para consultas de solo lectura.

5. **Control de acceso diferenciado**: Separar las interfaces de cliente (pública) y cocina (protegida con token estático) para garantizar que solo personal autorizado pueda modificar estados de pedidos.
...
#### **RN-006: Unicidad de Mesa por Sesión**

**Definición:** Un cliente puede tener solo un pedido activo por mesa a la vez.

**Implementación:** **NO IMPLEMENTADA EN CÓDIGO ACTUAL**

**Evidencia de ausencia:** No se encontró constraint unique en `(tableId, status)` ni validación en `OrderService.createOrder()` que verifique pedidos pendientes/en preparación para la misma mesa.

**Riesgo identificado:** Dos clientes en la misma mesa podrían crear pedidos duplicados accidentalmente. Frontend no previene esto, backend lo permite.

**Recomendación:** Validación explícita en `OrderValidator` o constraint de BD con partial unique index:
```sql
CREATE UNIQUE INDEX idx_one_active_order_per_table 
ON orders(table_id) 
WHERE status IN ('PENDING', 'IN_PREPARATION') AND deleted = false;
```
...
#### **PERFIL 3: Administrador/Gerente (Implícito)**

**Nota:** Este perfil NO está formalmente implementado con roles RBAC, pero se infiere de la existencia del módulo de reportes.

**Capacidades detectadas:**
- Consultar reportes de ventas por rango de fechas
- Ver métricas agregadas (ingresos totales, productos más vendidos)

**Ausencias críticas:**
- No hay autenticación requerida para acceder a `/reports/orders`
- No hay diferenciación entre gerente y empleado
- No hay control de acceso granular (cualquiera con la URL puede ver reportes)

**Rutas frontend:**
- `/reports/orders` - Dashboard de reportería

**Riesgo de seguridad identificado:** El endpoint de reportes es público. Un cliente malicioso que descubra la ruta `/reports/orders` puede acceder a información sensible del negocio (ingresos totales, productos más vendidos) sin autenticación.

**Recomendación:** Implementar autenticación similar a la de cocina con role `ADMIN` o integrar con sistema de SSO corporativo.

---

### Permisos y Limitaciones de cada Perfil

| Operación | Cliente | Cocina | Admin (No impl.) |
|-----------|---------|--------|------------------|
| **Ver menú** | ✅ | ✅ (indirectamente) | ✅ |
| **Crear pedido** | ✅ | ❌ | ❌ |
| **Ver pedido específico** | ✅ (solo propio) | ✅ (todos) | ✅ (todos) |
| **Cambiar estado pedido** | ❌ | ✅ (con restricciones de flujo) | ⚠️ No implementado |
| **Eliminar pedido** | ❌ | ✅ (soft delete) | ⚠️ No implementado |
| **Ver reportes** | ❌ | ❌ | ⚠️ Público (sin autenticación) |
| **Administrar productos** | ❌ | ❌ | ⚠️ No existe endpoint |
| **Administrar mesas** | ❌ | ❌ | ⚠️ No existe endpoint |
...
### 7.1 Deuda Técnica Identificada

#### **DT-001: Ausencia de Schema Registry**

**Severidad:** ALTA  
**Impacto:** Drift no controlado entre contratos de eventos

**Descripción:**
Los eventos AMQP (`OrderPlacedEvent`, `OrderReadyEvent`) se definen como clases Java duplicadas en cada servicio sin un contrato formal compartido. Esto causa:
- Validación manual de versiones en cada consumidor
- Riesgo de breaking changes no detectados en compile-time
- Imposibilidad de generación automática de consumers/producers

**Evidencia:**
- `order-service/OrderPlacedEvent.java` (producer)
- `kitchen-worker/OrderPlacedEvent.java` (consumer, definición duplicada)
- `report-service/OrderPlacedEvent.java` (consumer, definición duplicada)

**Recomendación:**
- Implementar Avro/Protobuf con Schema Registry (Confluent o Apicurio)
- Migrar a librería compartida `common-events` con clases generadas
- Añadir validation automático en pipeline CI

**Esfuerzo estimado:** 3-5 días de desarrollo + 2 días de testing

---

#### **DT-002: Seguridad de Cocina con Token Estático**

**Severidad:** ALTA  
**Impacto:** Riesgo de acceso no autorizado

**Descripción:**
El sistema usa un token estático compartido (`cocina123`) almacenado en `localStorage` del navegador y validado contra variable de entorno en backend. Problemas:
- Token no expira nunca (sin TTL)
- Mismo token para todos los usuarios de cocina (sin audit trail individual)
- Transmisión en header HTTP sin cifrado en ambientes sin HTTPS
- Token visible en código frontend compilado
...
```typescript
// kitchenAuth.ts
export function saveKitchenToken(token: string) {
  localStorage.setItem(STORAGE_KEY, token)
}
```

**Alternativa recomendada:**
- JWT con claims (`sub: user_id`, `role: kitchen_staff`)
- Expiración configurable (e.g., 8 horas de turno)
- Refresh tokens para renovación sin re-login
- Integración con OAuth2/OIDC (Keycloak, Auth0)

**Esfuerzo estimado:** 5-8 días (implementación completa de autenticación stateless)

---

#### **DT-003: Manejo Inconsistente de Errores HTTP**

**Severidad:** MEDIA  
**Impacto:** Experiencia de usuario degradada en flujos de error

**Descripción:**
Los tres servicios tienen estrategias diferentes de manejo de excepciones:

| Servicio | Error generico (500) | Errores validación (400) | Errores negocio (custom) |
|----------|---------------------|--------------------------|--------------------------|
| order-service | ✅ `GlobalExceptionHandler` | ✅ Bean Validation auto | ✅ Exceptions custom |
| kitchen-worker | ❌ No expone HTTP | N/A | Log + DLQ |
| report-service | ⚠️ try-catch inline | ⚠️ Cuerpo vacío | ❌ Sin `@RestControllerAdvice` |

**Problemas específicos:**
1. `report-service` retorna HTML de error default de Spring en casos no controlados
2. `MethodArgumentTypeMismatchException` no capturada (retorna 500 en lugar de 400)
3. Mensajes de error exponen detalles internos (nombres de columnas SQL, rutas de clases)

**Evidencia documental:** `docs/week-3-review/REST_API_AUDIT.md`, sección "Categorías de defectos funcionales"

**Recomendación:**
- Centralizar manejo de errores en biblioteca compartida
- Estructura de error unificada:
  ```json
  {
    "error": "INVALID_STATUS_TRANSITION",
    "message": "Cannot change status from READY to PENDING",
...
#### **DT-004: Ausencia de Rate Limiting**

**Severidad:** MEDIA  
**Impacto:** Vulnerabilidad a ataques DoS accidentales o maliciosos

**Descripción:**
Ningún endpoint expone rate limiting o throttling. Un cliente malicioso puede:
- Crear pedidos masivos (POST /orders en loop)
- Saturar tablero de cocina con polling agresivo
- Extraer datos completos de reportes sin restricción

**Escenario de abuso detectado:**
```python
# Script hipotético de abuso
...
**Mitigación recomendada:**
- Spring Boot Starter Bucket4j (token bucket algorithm)
- Límites sugeridos:
  - Cliente: 10 pedidos/minuto por IP
  - Cocina: 100 requests/minuto por token
  - Reportes: 30 requests/hora por IP

**Esfuerzo estimado:** 1-2 días (configuración + testing)

---

#### **DT-005: Testing Incompleto**

**Severidad:** MEDIA  
**Impacto:** Riesgo de regresiones no detectadas

**Cobertura actual:**
- `order-service`: 61 tests, cobertura ~75% (según configuración JaCoCo)
- `kitchen-worker`: 13 tests
- `report-service`: Sin información de cobertura documentada

**Tipos de test ausentes:**
1. **Contract testing:** No hay validación de compatibilidad de eventos entre servicios
2. **E2E tests:** No hay tests que validen flujo completo cliente → backend → cocina
3. **Performance tests:** No hay benchmarks de throughput/latency
4. **Chaos engineering:** No hay pruebas de resiliencia ante fallos de PostgreSQL/RabbitMQ

**Recomendación:**
- Implementar Pact para consumer-driven contracts
- Suite E2E con Playwright/Cypress (actualmente solo existe `scripts/smoke.sh`)
- JMeter/Gatling para load testing (target: 100 req/s en POST /orders)

**Esfuerzo estimado:** 8-12 días (implementación completa de test strategy)

---

### 7.2 Riesgos Operacionales

#### **RIESGO-001: Single Point of Failure en RabbitMQ**

**Probabilidad:** MEDIA  
**Impacto:** CRÍTICO

**Descripción:**
RabbitMQ corre en contenedor único sin cluster ni réplicas. Si el broker falla:
- Nuevos pedidos se crean en `order-service` pero jamás llegan a cocina
- La inconsistencia entre `restaurant_db` y `kitchen_db` puede durar horas
- No hay persistencia garantizada de mensajes (durable queues no configuradas)

**Escenario de fallo:**
1. Cliente crea pedido → 201 Created
2. Backend publica evento a RabbitMQ
3. RabbitMQ cae ANTES de que `kitchen-worker` consuma
4. Evento se pierde (no hay persistent storage)
5. Orden queda en estado PENDING indefinidamente

**Mitigación propuesta:**
- RabbitMQ cluster con 3 nodos (quorum queues en RabbitMQ 3.12+)
- Configurar `durable: true` en queues y exchanges
- Implementar circuit breaker en publisher (Resilience4j)

**Esfuerzo:** 3-5 días

---

#### **RIESGO-002: Ausencia de Monitoreo y Alertas**

**Probabilidad:** ALTA  
**Impacto:** MEDIO

**Descripción:**
No se encontraron integraciones con:
- Prometheus/Grafana (métricas de negocio)
- ELK/Loki (logging centralizado)
- Sentry/Rollbar (error tracking)
- PagerDuty/OpsGenie (incident management)

**Métricas críticas no rastreadas:**
- Órdenes en estado PENDING > 10 minutos
- Tamaño de DLQ (debería alertar si > 5 mensajes)
- Latencia P95 de POST /orders
- Tasa de error 5xx

**Recomendación:**
- Añadir Spring Boot Actuator + Micrometer
- Dashboard Grafana con métricas de negocio:
  - Throughput de órdenes/hora
  - Tiempo promedio PENDING → READY
  - Top 10 productos más vendidos (tiempo real)

**Esfuerzo:** 4-6 días

---

#### **RIESGO-003: Limitaciones de Escalamiento Horizontal**

**Probabilidad:** MEDIA  
**Impacto:** MEDIO

**Descripción:**
Si bien los microservicios son stateless (candidatos a escalar horizontalmente), existen limitaciones:
- Frontend no usa load balancer (hardcoded `http://localhost:8080`)
- Sesión de carrito en React Context (se pierde en refresh, no distribuida)
- Polling de cocina aumenta linearlmente con cada instancia del frontend

**Cálculo de carga:**
- 1 terminal de cocina: 20 req/min
- Sistema horario pico con 10 meseros + 5 terminales cocina: 300 req/min
- PostgreSQL puede manejar, pero sin índices adecuados podría degradarse

**Recomendación:**
- Implementar Redis para sesión de carrito (distribuida entre pods)
- Envoy/Nginx como API Gateway con balanceo round-robin
- Índices compuestos en queries frecuentes:
  ```sql
  CREATE INDEX idx_orders_active 
  ON orders(status, deleted) 
  WHERE deleted = false;
  ```
...
### 7.3 Observaciones de Arquitectura

#### **OBS-001: Patrón Microservices vs. Monolito Distribuido**

**Análisis:**
El sistema exhibe características de **Distributed Monolith** disfrazado de microservicios:
- Los tres servicios comparten enum `OrderStatus` sin contrato formal
- Despliegue coordinado requerido para cambios en eventos
- No hay versioning semántico independiente por servicio

**Beneficios actuales de la separación:**
✅ Database-per-service (aislamiento de datos)  
✅ Tecnologías específicas por contexto (e.g., kitchen-worker sin HTTP)  
✅ Escalado independiente (teóricamente)

**Costos de la distribución:**
❌ Complejidad operacional (3 bases de datos + 1 broker)  
❌ Debugging más difícil (trazas distribuidas no implementadas)  
❌ Eventual consistency (complejidad en UX)

**Recomendación:**
- Evaluar migración a **Modular Monolith** con módulos bien definidos
- Si se mantiene microservicios, implementar:
  - OpenTelemetry para distributed tracing
  - Saga pattern para transacciones distribuidas (no hay compensación en código actual)

---

#### **OBS-002: CQRS Bien Aplicado en Report Service**

**Análisis:**
El `report-service` es un buen ejemplo de **CQRS (Command Query Responsibility Segregation)**:
- Modelo de escritura: Eventos consumidos desde RabbitMQ
- Modelo de lectura: Agregaciones precalculadas en `ReportAggregationService`
- Base de datos desnormalizada optimizada para queries

**Ventajas observadas:**
✅ Consultas de reportes no impactan performance de `order-service`  
✅ Esquema de lectura simplificado (join-free queries)  
✅ Fácil añadir nuevas proyecciones (e.g., reportes por categoría de producto)

**Mejora propuesta:**
- Materializar agregaciones en tabla separada con refresh programado:
  ```sql
...
#### **SEC-001: Inyección SQL Prevenida (Low Risk)**

**Análisis:** Spring Data JPA con named parameters previene SQL injection:
```java
@Query("SELECT o FROM Order o WHERE o.id = :id AND o.deleted = false")
Optional<Order> findByIdAndDeletedFalse(@Param("id") UUID id);
```

✅ Sin vulnerabilidad de inyección detectada

---

#### **SEC-002: CORS Configurado Restrictivamente**
...
✅ Solo permite frontend local, bloquea orígenes arbitrarios

**Mejora para producción:** Configurar dominio real:
```
CORS_ALLOWED_ORIGINS=https://pedidos.restaurante.com
```

---

#### **SEC-003: Datos Sensibles en Logs (Medium Risk)**
...
⚠️ Logs contienen IDs de pedidos que podrían correlacionarse con clientes

**Recomendación:** Sanitizar logs en producción, usar log levels apropiados (DEBUG para detalles sensibles).
...
**Nivel actual: DEVELOPMENT/STAGING**

El sistema presenta una base sólida con patrones arquitectónicos bien aplicados (CQRS, Event-Driven, Hexagonal parcial) pero requiere hardening significativo para producción:

✅ **Fortalezas:**
- Separación de contextos bounded (Order, Kitchen, Reports)
- Soft delete para auditoría
- Cobertura de testing aceptable en `order-service`
...
- Resiliencia de messaging (RabbitMQ SPOF)
- Rate limiting
...
#### **Fase 1: Security Hardening (2-3 semanas)**
1. Reemplazar token estático por JWT
2. Implementar rate limiting en todos los endpoints
3. Añadir HTTPS obligatorio (TLS 1.3+)
4. Sanitizar logs de datos sensibles

#### **Fase 2: Operational Excellence (3-4 semanas)**
1. Prometheus + Grafana para métricas
2. ELK/Loki para logging centralizado
3. Cluster RabbitMQ con 3 nodos
4. Health checks y readiness probes para Kubernetes

#### **Fase 3: Testing & Quality (2-3 semanas)**
1. Suite de contract testing con Pact
2. E2E tests con Playwright
3. Load testing con JMeter (target: 100 req/s)
4. Alcanzar 85% cobertura en todos los servicios

#### **Fase 4: Feature Completeness (4-6 semanas)**
1. Módulo de administración (CRUD de productos/mesas)
2. Integración con sistema de pagos
3. Dashboard analytics en tiempo real
4. Notificaciones push (WebSocket/SSE)

---

### Veredicto Final

El **Sistema de Pedidos de Restaurante** es un proyecto académico/MVP bien estructurado con excelente potencial para escalar a un producto comercial. La arquitectura de microservicios está bien justificada para el dominio de negocio (separación cliente/cocina/administración) y las tecnologías elegidas son apropiadas para el stack moderno (Spring Boot, React, PostgreSQL, RabbitMQ).

**Principales valor agregados del sistema:**
1. Automatización completa del flujo de pedidos
2. Visibilidad en tiempo real del estado de cocina
3. Fundación sólida para analytics de negocio
4. Arquitectura extensible para funcionalidades futuras

---

**Documento generado por:** Análisis automatizado de código fuente  
**Total de archivos analizados:** 150+ (Java, TypeScript, YAML, SQL)  
...
### Apéndice A: Endpoints Completos del Sistema
...
- `PATCH /orders/{id}/status` - Cambiar estado (requiere kitchen token)
- `DELETE /orders/{id}` - Soft delete de pedido (requiere kitchen token)
- `DELETE /orders` - Soft delete masivo (requiere kitchen token)

**Report Service (8082):**
- `GET /reports` - Reporte de ventas con filtros de fecha

**Frontend Routes:**
- `/` - Página de bienvenida
- `/client/table` - Selección de mesa
- `/client/menu` - Menú digital
- `/client/cart` - Carrito de compras
- `/client/confirm/:orderId` - Confirmación de pedido
- `/client/status/:orderId` - Estado de pedido
- `/kitchen` - Login de cocina
- `/kitchen/board` - Tablero kanban
- `/reports/orders` - Dashboard de reportes

---

### Apéndice B: Entidades de Base de Datos

**restaurant_db:**
- `orders` (id, table_id, status, created_at, updated_at, deleted, deleted_at)
- `order_items` (id, order_id, product_id, quantity, note)
- `products` (id, name, description, price, category, image_url, is_active)

**kitchen_db:**
- `kitchen_orders` (id, table_id, status, created_at, updated_at)

**report_db:**
- `report_orders` (id, table_id, status, created_at, received_at)
- `report_order_items` (id, order_id, product_id, quantity, price)

---

### Apéndice C: Variables de Entorno Críticas
